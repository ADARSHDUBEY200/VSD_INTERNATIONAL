...
import { listPublishedBlogs, type BlogCard } from '@/lib/blogPresenter';
import { SITE_URL, ORG_ID } from '@/lib/config';

// The listing is CMS-backed — regenerate periodically so newly published
// posts appear without a full rebuild.
...
/** Pick a category emoji from the CMS category label. */
function categoryIcon(category: string): string {
  ...
  if (c.includes('bakery') || c.includes('confection')) return '🥐';
...
function buildSchema(posts: BlogCard[]) {
  return {
...
export default async function BlogIndexPage() {
  ...
  // Distinct categories present across published posts, with counts.
  ...
  return (
    <>
      <script
        ...
        dangerouslySetInnerHTML={{ __html: JSON.stringify(buildSchema(posts)) }}
      />
      ...
      {posts.length === 0 ? (
        ...
        <section style={{ padding: '5rem 0', background: '#FFFFFF' }}>
          <div className="container mx-auto" style={{ maxWidth: '80rem', padding: '0 1.25rem', textAlign: 'center' }}>
            ...
            <h2 style={{ fontFamily: 'var(--font-playfair)', fontSize: '1.5rem', fontWeight: 700, color: 'var(--text-dark)', marginBottom: '0.75rem' }}>
              No articles published yet
            </h2>
            <p style={{ fontFamily: 'var(--font-inter)', fontSize: '0.9375rem', color: 'var(--text-muted)' }}>
              New commercial kitchen guides are on the way — check back soon.
            </p>
          </div>
        </section>
      ) : (
                  ...
                  {categories.map(([category, count]) => (
                    ...
                    >
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '0.5rem' }}>
                        <span style={{ fontSize: '1.5rem' }} aria-hidden="true">{categoryIcon(category)}</span>
                        <h2
                          style={{
                            fontFamily: 'var(--font-playfair)',
                            fontSize: '1.0625rem',
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
                ...
                <div style={{ borderRadius: '12px', overflow: 'hidden', background: 'var(--charcoal-light)', position: 'relative', minHeight: '280px' }}>
                  {featuredPost.mainImage ? (
                    <Image src={featuredPost.mainImage} alt={featuredPost.title} fill style={{ objectFit: 'cover' }} sizes="(max-width: 1024px) 100vw, 50vw" />
                  ) : (
                    <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(135deg, #0F0D0A 0%, rgba(42,34,21,0.9) 50%, #0F0D0A 100%)' }}>
                      <div style={{ textAlign: 'center' }}>
                        <span style={{ display: 'block', fontSize: '3rem', marginBottom: '0.5rem' }}>{categoryIcon(featuredPost.category)}</span>
                ...
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1rem' }}>
                    ...
                    <span style={{ fontSize: '0.75rem', fontFamily: 'var(--font-inter)', color: 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: '0.25rem' }}>
                      <Clock size={11} aria-hidden="true" /> {featuredPost.readTime}
                    </span>
                  </div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '2rem', flexWrap: 'wrap' }}>
                    <span style={{ fontSize: '0.8125rem', fontFamily: 'var(--font-inter)', color: 'var(--text-muted)' }}>
                      By <strong style={{ color: 'var(--text-dark)' }}>{featuredPost.author}</strong>
                    </span>
                    <span style={{ color: 'var(--border)' }}>·</span>
                    <time dateTime={featuredPost.publishedAt} style={{ fontSize: '0.8125rem', fontFamily: 'var(--font-inter)', color: 'var(--text-muted)' }}>
                      {new Date(featuredPost.publishedAt).toLocaleDateString('en-IN', { day: 'numeric', month: 'long', year: 'numeric' })}
                    </time>
                  </div>
                  ...
                  {recentPosts.map((post) => (
                    ...
                    >
                      ...
                      <div style={{ background: 'var(--charcoal-light)', position: 'relative', height: '160px' }}>
                        {post.mainImage ? (
                          <Image src={post.mainImage} alt={post.title} fill style={{ objectFit: 'cover' }} sizes="(max-width: 640px) 100vw, 50vw" />
                        ) : (
                          <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(135deg, #0F0D0A 0%, rgba(42,34,21,0.85) 100%)' }}>
                            <span style={{ fontSize: '2rem' }} aria-hidden="true">{categoryIcon(post.category)}</span>
                          </div>
                        )}
                        ...
                        <span style={{ position: 'absolute', top: '0.75rem', left: '0.75rem', padding: '0.2rem 0.6rem', borderRadius: '100px', background: 'rgba(15,13,10,0.75)', border: '1px solid rgba(201,168,76,0.3)', fontSize: '0.65rem', fontFamily: 'var(--font-inter)', fontWeight: 600, color: 'var(--gold-bright)', letterSpacing: '0.05em' }}>
                          {post.category}
                        </span>
                      </div>
                      ...
                      <div style={{ padding: '1.25rem 1.5rem' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.625rem', fontSize: '0.75rem', fontFamily: 'var(--font-inter)', color: 'var(--text-muted)' }}>
                          <time dateTime={post.publishedAt}>{formatMonthYear(post.publishedAt)}</time>
                          <span>·</span>
